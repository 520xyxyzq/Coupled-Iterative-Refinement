...
def drawDetections(
    ...
    if hasattr(detections, "poses"):
        ...
        img_size = torch.tensor(image.shape[-2:])
        img_size = img_size.to(image.device).to(torch.float).view(1, 2)
        img_to_draw = image.detach().clone()
        for obj_label, pose, color in zip(labels, poses, colors_obj_rgb):
            rgb_ren, _, _ = renderer(
                [obj_label], pose.unsqueeze(0), intrinsics, img_size
            ) # 1x3xHxW
