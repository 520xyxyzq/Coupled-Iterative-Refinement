...
def read_depth(depth_path: Path, depth_scale: float, interpolate=False):
    ...
    if interpolate:  
        # interpolating the missing depth values takes about 0.7s,scipy is slow
...
def main():
    ...
    parser.add_argument(
        '--scene_dir', type=Path, required=True,
        help="A folder with an rgb/ subdir, a scene_camera.json, "
        + "and (Optionally) a depth/ subdir"
    )
    parser.add_argument(
        '--output_dir', type=Path, default="qualitative_output",
        help="The directory to save qualitative output"
    )
    parser.add_argument(
        '-o', '--override', nargs='+', type=str, default=[],
        help="gin-config settings to override"
    )
    parser.add_argument(
        '--load_weights', type=str, required=True,
        help='path to the model weights to load'
    )
    parser.add_argument(
        '--num_outer_loops', type=int, default=2,
        help="number of outer-loops in each forward pass"
    )
    parser.add_argument(
        '--num_inner_loops', type=int, default=10,
        help="number of inner-loops in each forward pass"
    )
    parser.add_argument(
        '--num_solver_steps', type=int, default=3,
        help="number of BD-PnP solver steps per inner-loop"
        + "(doesn't affect Modified BD-PnP)"
    )
    parser.add_argument(
        '--obj_models', required=True, help="which object models to use",
        choices=['ycbv', 'tless', 'lmo', 'hb', 'tudl', 'icbin', 'itodd']
    )
    parser.add_argument(
        '--rgb_only', action='store_true', help="use the RGB-only model"
    )
    parser.add_argument(
    ...
    gin.parse_config_files_and_bindings(
        [
            "configs/base.gin",
            f"configs/test_{args.obj_models}" +
            f"_{'rgb' if args.rgb_only else 'rgbd'}.gin"
        ], args.override
    )
    ...
    model.eval()
    # Loading Renders. This gets cached so it's only slow the first time.
    Pytorch3DRenderer()

    print(f"\n\nSaving images output to {args.scene_dir}/candidates.csv\n\n")
    # args.output_dir.mkdir(exist_ok=True)

    if not args.scene_dir.exists():
        raise FileNotFoundError(
            f"The directory {args.scene_dir} doesn't exist." +
            " Download a sample scene using ./download_sample.sh" +
            " or set --scene_dir to a BOP scene directory."
        )
    if not (args.scene_dir / "rgb").exists():
        raise FileNotFoundError(
            f"The directory {args.scene_dir / 'rgb'} doesn't exist."
        )
    if not (args.scene_dir / "scene_camera.json").exists():
        raise FileNotFoundError(
            f"The file {args.scene_dir / 'scene_camera.json'} doesn't exist."
        )
    if not args.rgb_only and not (args.scene_dir / "depth").exists():
        raise FileNotFoundError(
            f"The directory {args.scene_dir / 'depth'} doesn't exist," +
            " and --rgb_only isn't used."
        )

    # Read camera intrinsics
    scene_cameras = json.loads((
        args.scene_dir / "scene_camera.json"
    ).read_text())
    image_loop = list(scene_cameras.items())
    # Read image file names
    image_names = list(args.scene_dir.glob("rgb/*.png"))
    image_indices = [int(img_n.name[:-4]) for img_n in image_names]
    image_index_name_map = dict(zip(image_indices, image_names))
    ...
    for image_index, (frame_id, scene_camera) in enumerate(image_loop):
        camera_intrinsics = torch.as_tensor(
            scene_camera['cam_K'], device='cuda', dtype=torch.float32
        ).view(1, 3, 3)
        depth_scale = scene_camera['depth_scale']
        assert int(frame_id) in image_index_name_map, \
            f"Error: No image named ...{frame_id}.png in {args.scene_dir}/rgb"
        rgb_path = image_index_name_map[int(frame_id)]
        images = imageio.v2.imread(rgb_path)
        render_resolution = torch.tensor(
            images.shape[:2], device='cuda', dtype=torch.float32
        ).view(1, 2) / 2
        images = torch.as_tensor(
            images, device='cuda', dtype=torch.float32
        ).permute(2, 0, 1).unsqueeze(0) / 255
        if args.rgb_only:
            interpolated_depth = torch.zeros_like(images[:, 0])
        else:
            depth_path = args.scene_dir / "depth" / rgb_path.name
            interpolated_depth = read_depth(
                depth_path, depth_scale, interpolate=True
            )
            interpolated_depth = torch.as_tensor(
                interpolated_depth, device='cuda', dtype=torch.float32
            ).unsqueeze(0)

        # Generate candidate detections using a Mask-RCNN
        if not args.read_det:
            detections = detector.get_detections(
                images=images, detection_th=0.3
            )
        else:
        ...
        data_TCO_init = generate_pose_from_detections(
            detections=detections, K=camera_intrinsics
        )
        ...
        img_preds = []
        tqdm_iter = tqdm(
            list(detections.infos.iterrows()),
            desc=f"Predicting poses for {len(detections)} detected objects" +
            f" in image {image_index+1}/{len(image_loop)}"
        )
        for obj_idx, (_, obj_label, _) in tqdm_iter:
            ...
            images_cropped, K_cropped, _, _, masks_cropped, depths_cropped = \
                crop_inputs(
            ...
            mrcnn_rendered_rgb, _, _ = Pytorch3DRenderer()(
                [obj_label], mrcnn_pose, K_cropped, render_resolution
            )
            ...
            images_input = torch.cat((
                images_cropped, mrcnn_rendered_rgb
            ), dim=1)
            current_pose_est = run_efficientnet(
                images_input, mrcnn_pose, K_cropped
            )
            ...
            for outer_loop_idx in range(args.num_outer_loops):
                ...
                images_cropped, K_cropped, _, _, masks_cropped, depths_cropped\
                    = crop_inputs(
                        images=images.clone(), K=camera_intrinsics,
                        TCO=current_pose_est, labels=[obj_label],
                        masks=mrcnn_mask, sce_depth=interpolated_depth,
                        render_size=render_resolution.squeeze().cpu().numpy()
                    )

                # Render additional viewpoints
                input_pose_multiview = get_perturbations(
                    current_pose_est
                ).flatten(0, 1)
                ...
                rendered_rgb, rendered_depth, _ = Pytorch3DRenderer()(
                    label_rep, input_pose_multiview, K_rep, res_rep
                )

                # Forward pass
                def combine(a, b): return torch.cat((
                    a.unflatten(0, (1, Nr)), b.unsqueeze(1)
                ), dim=1)
                ...
                outputs = model(
                    Gs=pose_input, images=images_input,
                    depths_fullres=depths_input, masks_fullres=masks_input,
                    intrinsics_mat=K_input, labels=[obj_label],
                    num_solver_steps=args.num_solver_steps,
                    num_inner_loops=args.num_inner_loops
                )
                current_pose_est = SE3(
                    outputs['Gs'][-1].contiguous()[:, -1]
                ).matrix()
            ...
            batch_preds = PandasTensorCollection(
                data_TCO_init.infos[obj_idx:obj_idx+1],
